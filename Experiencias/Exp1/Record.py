...
class Record:
    ...
    def __init__(self, values, pid, extension='csv'):
        ...
        self.fields  = ['time', 'h_1', 'h_2', 'h_3', 'h_4', 'v_1', 'v_2', 'r_1', 'r_2', \
                        'e_P_1', 'e_I_1', 'e_D_1', 'k_P_1', 'k_I_1', 'k_D_1', 'k_w_1', 'u_1', 'u0_1', 'setPoint_1',\
                        'e_P_2', 'e_I_2', 'e_D_2', 'k_P_2', 'k_I_2', 'k_D_2', 'k_w_2', 'u_2', 'u0_2', 'setPoint_2']
        self.t = None
        ...
        if self.ext == 'csv':
            ...
            with open('RecordData.csv', 'a', newline='') as csvfile:
                ...
                while not self.stopped:
                    ...
                    rows.extend(["{:.3f}".format(i) for i in states1.values()][0:-1])
                    states2 = self.pid.H2.status()
                    rows.extend(["{:.3f}".format(i) for i in states2.values()][0:-1])
            ...
            with open(self.namenpy+'.npy', 'wb') as f:
                ...
                while not self.stopped:
                    ...
                    print(l1)
                    states1 = self.pid.H1.status()
                    l1.extend(list(states1.values())[0:-1])
                    print(l1)
                    states2 = self.pid.H2.status()
                    l1.extend(list(states2.values())[0:-1])
                    print(l1)
                    l1.append(time.time()-old_time)
                    print(l1)
