...
for i = 1:2:3
    o(i) = plot(states.Time, states.data(:, i), ':.', 'DisplayName', sprintf('Estado %d', i), 'MarkerSize', 12);
end
% plot(states.Time, x0(1)*ones(size(states.Time)), 'r--');
leg = legend({'$\phi$', '$\theta$'}, 'location', 'best');
...
xlim([0 50])
Ylim =  get(gca, 'YLim');
text(30,Ylim(1)+diff(Ylim)/15,['J=' num2str(J, '%.2f')],'FontSize',20);
%%
